...
Store.prototype.renderToPage = function () {
  ...
  for (var i = 0; i < this.storeHoursOpen.length; i++) {
    ...
    var renderedItems =
      this.storeHoursOpen[i] + ": " + this.cookieSalesPerHour[i] + " cookies"; // recreating and reassigning var render
...
Store.prototype.renderTableRow = function () {
  //1. something to target( id )
  var tableEltoTarget = document.getElementById('storeTable');
  //2. Create Content
  //make a row, then make 13 td, then put content in tds, then attach them to tr
  //  <tr>
  //   <td>data</td>
  //   <td>data</td>
  //   <td>data</td>
  ...
  var newThEl = document.createElement('td');
